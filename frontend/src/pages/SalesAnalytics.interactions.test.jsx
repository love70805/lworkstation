...
const rows=Array.from({length:15},(_,i)=>({store:'甲',platformSku:`SKU-${String(i).padStart(2,'0')}`,quantity:i+1,amount:15-i,sourceAddedDate:date,attribute:'红色',activityStatus:'known',activityRaw:'合成活动'.repeat(40)}));
...
it('paginates only selected-day products, searches without changing totals and lazily expands activity originals',async()=>{
 await click(dayButton());expect(container.querySelectorAll('tbody tr')).toHaveLength(12);
 expect(container.querySelector('tbody tr').textContent).toContain('SKU-00');
 ...
 const original=container.querySelector('.sales-activities details');
 await act(async()=>{original.open=true;original.dispatchEvent(new Event('toggle'));});
 expect(container.querySelector('.sales-activity-original').textContent).toBe(rows[0].activityRaw);
 ...
 expect(container.querySelector('tbody tr').textContent).toContain('SKU-14');
 const before=container.querySelector('.sales-details-heading p').textContent;
 await act(async()=>Simulate.change(container.querySelector('.sales-details-controls input'),{target:{value:'SKU-03'}}));
