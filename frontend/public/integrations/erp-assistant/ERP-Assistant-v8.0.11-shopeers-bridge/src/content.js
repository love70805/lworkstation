(function () {
    ...
    const RESULT_CACHE_KEY = 'latest_cost_result_v3';
    const PREFIX = '[ERP Assistant]';
    const EXTENSION_VERSION = '8.0.11';
    ...
    function dispatchCostResults(results, meta) {
        ...
        if (hasPendingCostAnomalies(results)) return false;
    ...
    function anomalyReasons(detail) {
        return Array.isArray(detail && detail.anomalyReasons)
            ? detail.anomalyReasons.filter(Boolean)
            : [];
    }

    function formatAnomalyReasons(reasons) {
        return (Array.isArray(reasons) ? reasons : [])
            .map((reason) => resultPolicy.anomalyReasonLabel(reason))
            .filter(Boolean)
            .join('；');
    }

    function resultCostAnomaly(result) {
        if (result && result.costAnomaly) return result.costAnomaly;
        return resultPolicy.summarizeCostAnomalies(result && result.details);
    }

    function hasPendingCostAnomalies(results) {
        return (Array.isArray(results) ? results : []).some((result) => (
            Number(resultCostAnomaly(result).pendingCount || 0) > 0
        ));
    }

    function buildAnomalyMeta(results) {
        const summaries = (Array.isArray(results) ? results : []).map(resultCostAnomaly);
        return {
            anomalyCount: summaries.reduce((sum, summary) => sum + Number(summary.count || 0), 0),
            anomalyPendingCount: summaries.reduce((sum, summary) => sum + Number(summary.pendingCount || 0), 0),
            anomalyConfirmedCount: summaries.reduce((sum, summary) => sum + Number(summary.confirmedCount || 0), 0)
        };
    }

    function recalculateAggregatedResult(result, details) {
        const nextDetails = (Array.isArray(details) ? details : []).map((detail) => {
            const qty = Number(detail && detail.qty);
            const unitPrice = Number(detail && detail.unitPrice);
            const totalPrice = Number.isFinite(qty) && Number.isFinite(unitPrice) ? qty * unitPrice : 0;
            return Object.assign({}, detail, {
                unitPrice: Number.isFinite(unitPrice) ? unitPrice.toFixed(4) : '',
                price: totalPrice.toFixed(2)
            });
        });
        const totalQty = nextDetails.reduce((sum, detail) => sum + Number(detail.qty || 0), 0);
        const totalPrice = nextDetails.reduce((sum, detail) => sum + Number(detail.price || 0), 0);
        const costAnomaly = resultPolicy.summarizeCostAnomalies(nextDetails);
        return Object.assign({}, result, {
            details: nextDetails,
            totalQty,
            totalPrice: totalPrice.toFixed(2),
            unitCost: totalQty > 0 ? (totalPrice / totalQty).toFixed(4) : '0.0000',
            selectedRecordIds: nextDetails.map((detail) => detail.recordId).filter(Boolean),
            costAnomaly,
            anomalyCount: costAnomaly.count,
            anomalyPendingCount: costAnomaly.pendingCount,
            anomalyConfirmed: costAnomaly.pendingCount === 0
        });
    }
    ...
    function aggregateDetails(details, now) {
        ...
        buckets.forEach((records, warehouseSku) => {
            ...
            const selected = resultPolicy.detectCostAnomalies(selectCostRecords(records));
            ...
            const costAnomaly = resultPolicy.summarizeCostAnomalies(selected);
            ...
            results.push({
                ...
                details: selected.map((record) => ({
                    ...
                    anomalyReferenceUnitPrice: record.anomalyReferenceUnitPrice,
                    anomalyConfirmed: record.anomalyConfirmed,
                    anomalyConfirmedAt: null,
    ...
    async function runCalculation(filters, run) {
        ...
        const anomalyMeta = buildAnomalyMeta(scopedState.results);
        return {
            ...
            meta: {
                ...
                anomalyCount: anomalyMeta.anomalyCount,
                anomalyPendingCount: anomalyMeta.anomalyPendingCount,
                anomalyConfirmedCount: anomalyMeta.anomalyConfirmedCount,
    ...
    async function calculate() {
        ...
        const state = await runCalculation(filters, run);
            ...
            const dispatched = dispatchCostResults(lastResults, lastMeta);
            expandedRows.clear();
            ...
            updateActionState();
            hideLoading();
            showToast('已完成 ' + lastResults.length + ' 个仓库 SKU 核算' + (dispatched ? '' : '，存在成本异常，等待人工修正确认'));
        } catch (error) {
        ...
        results.forEach((result) => {
            ...
            mappings.forEach((mapping) => {
                rows.push({
                    ...
                    anomalyCount: resultCostAnomaly(result).count,
                    anomalyReasons: resultCostAnomaly(result).reasons.map((reason) => resultPolicy.anomalyReasonLabel(reason)).join('；'),
                    anomalyConfirmed: resultCostAnomaly(result).pendingCount === 0,
                    anomalyRecords: resultCostAnomaly(result).records
                });
            });
        });
        return rows;
    ...
    async function copyCosts() {
        ...
        if (hasPendingCostAnomalies(lastResults)) {
            showToast('存在未确认的成本异常，请先逐条修正并确认。', 'error');
            return;
        }
        const lines = ['平台SKU\t平台SKC\t仓库SKU\t1688单号\t单件平均成本\t供应商1688链接\t成本异常\t异常原因\t人工确认\t异常审计JSON'];
        buildExportRows(lastResults).forEach((row) => {
            lines.push([row.platformSku, row.platformSkc, row.warehouseSku, row.orderNumber, row.unitCost, row.supplier1688Url, row.anomalyCount, row.anomalyReasons, row.anomalyConfirmed ? '是' : '否', JSON.stringify(row.anomalyRecords)].join('\t'));
        });
        try {
    ...
    function exportCsv() {
        ...
        const headers = [
            '仓库SKU', '平台SKU', '平台SKC', '单号类型', '1688单号', '产品名称', '供应商', '供应商1688链接', '成本异常', '异常原因', '人工确认', '异常审计JSON',
            '核算次数', '核算日期范围', '总采购量', '总采购价(￥)', '单件平均成本'
        ];
        const lines = [headers.map(csvCell).join(',')];
        buildExportRows(lastResults).forEach((row) => {
            lines.push([
                row.warehouseSku, row.platformSku, row.platformSkc, row.sourceType, row.orderNumber, row.name, row.supplierName, row.supplier1688Url, row.anomalyCount, row.anomalyReasons, row.anomalyConfirmed ? '是' : '否', JSON.stringify(row.anomalyRecords),
    ...
    function renderDetail(result) {
        const rows = result.details.map((detail) => {
            const reasons = anomalyReasons(detail);
            const hasAnomaly = reasons.length > 0;
            const anomalyState = hasAnomaly
                ? (detail.anomalyConfirmed
                    ? '<span class="erpa-anomaly-badge erpa-anomaly-confirmed">已人工修正</span><small class="erpa-anomaly-note">原价 ￥' + escapeHtml(detail.originalUnitPrice) + ' · ' + escapeHtml(formatAnomalyReasons(reasons)) + '</small>'
                    : '<span class="erpa-anomaly-badge erpa-anomaly-pending">待确认</span><small class="erpa-anomaly-note">' + escapeHtml(formatAnomalyReasons(reasons)) + '</small>')
                : '<span class="erpa-muted">正常</span>';
            return '<tr class="' + (hasAnomaly ? (detail.anomalyConfirmed ? 'erpa-detail-confirmed' : 'erpa-detail-anomaly') : '') + '"><td>' + escapeHtml(detail.date) + '</td>' +
            ...
            '<td class="erpa-anomaly-cell">' + anomalyState + '</td>' +
        ...
        return '<tr class="erpa-detail-row"><td colspan="9"><div class="erpa-detail-wrap">' +
            ...
            '<td>' + (resultCostAnomaly(result).pendingCount > 0 ? '<span class="erpa-anomaly-badge erpa-anomaly-pending">预览成本</span>' : '<span class="erpa-anomaly-badge erpa-anomaly-confirmed">正式成本</span>') + '</td>' +
    ...
    function renderResults() {
        ...
        body.innerHTML = results.map((result) => {
            ...
            const anomaly = resultCostAnomaly(result);
            const anomalyClass = anomaly.pendingCount > 0 ? ' erpa-result-anomaly' : (anomaly.count > 0 ? ' erpa-result-confirmed' : '');
            const costState = anomaly.pendingCount > 0
                ? '<span class="erpa-anomaly-badge erpa-anomaly-pending">待确认 ' + anomaly.pendingCount + '</span>'
                : (anomaly.count > 0 ? '<span class="erpa-anomaly-badge erpa-anomaly-confirmed">已修正</span>' : '');
            const row = '<tr class="erpa-result-row' + (expanded ? ' erpa-expanded' : '') + anomalyClass + '" data-sku="' + escapeHtml(key) + '">' +
    ...
    function renderStatus() {
        ...
        status.innerHTML =
            ...
            (lastMeta.anomalyPendingCount ? '<span class="erpa-status-item erpa-status-danger">成本异常待确认 <strong>' + lastMeta.anomalyPendingCount + '</strong></span>' : '') +
            (lastMeta.anomalyConfirmedCount ? '<span class="erpa-status-item erpa-status-ok">已人工修正 <strong>' + lastMeta.anomalyConfirmedCount + '</strong></span>' : '') +
    ...
    function pendingAnomalyEntries() {
        const entries = [];
        lastResults.forEach((result) => {
            (Array.isArray(result.details) ? result.details : []).forEach((detail) => {
                if (anomalyReasons(detail).length > 0 && !detail.anomalyConfirmed) {
                    entries.push({ result, detail });
                }
            });
        });
        return entries;
    ...
    function renderAnomalyBanner() {
        ...
        if (!banner) return;
        const pending = pendingAnomalyEntries();
        if (pending.length === 0) {
            banner.className = 'erpa-anomaly-banner';
            banner.innerHTML = '';
            return;
        }
        const total = lastResults.reduce((sum, result) => sum + Number(resultCostAnomaly(result).count || 0), 0);
        banner.className = 'erpa-anomaly-banner erpa-visible';
        banner.innerHTML = '<span class="erpa-anomaly-copy"><strong>发现 ' + pending.length + ' 条成本异常，暂不能作为正式成本</strong>' +
            '<small>异常总数 ' + total + ' 条。请逐条改为正确采购单价并点击“确认修正”，确认完成后才会允许复制、导出和自动回传。</small></span>' +
            '<button class="erpa-button erpa-button-warning" id="erpa-review-anomalies" type="button">逐条修正并确认</button>';
    }

    function reviewCostAnomalies() {
        const pending = pendingAnomalyEntries();
        if (pending.length === 0) return;
        if (typeof window.prompt !== 'function') {
            showToast('当前浏览器不支持人工修正对话框，请使用最新版 Chrome。', 'error');
            return;
        }

        const nextResults = lastResults.map((result) => Object.assign({}, result, {
            details: (Array.isArray(result.details) ? result.details : []).map((detail) => Object.assign({}, detail))
        }));
        let confirmedCount = 0;
        let cancelled = false;

        outer:
        for (let resultIndex = 0; resultIndex < nextResults.length; resultIndex += 1) {
            const result = nextResults[resultIndex];
            for (let detailIndex = 0; detailIndex < result.details.length; detailIndex += 1) {
                const detail = result.details[detailIndex];
                const reasons = anomalyReasons(detail);
                if (reasons.length === 0 || detail.anomalyConfirmed) continue;

                let corrected = null;
                while (corrected === null) {
                    const reference = detail.anomalyReferenceUnitPrice == null ? '无明确多数价' : Number(detail.anomalyReferenceUnitPrice).toFixed(4);
                    const answer = window.prompt(
                        '仓库 SKU：' + result.warehouseSku + '\n' +
                        '采购日期：' + detail.date + '\n' +
                        '原采购单价：' + detail.unitPrice + ' 元\n' +
                        '异常原因：' + formatAnomalyReasons(reasons) + '\n' +
                        '多数参考价：' + reference + '\n\n' +
                        '请输入人工确认后的正确单价（必须大于 0、不能等于 1，且与原价不同）：',
                    ...
                    if (!Number.isFinite(number) || number <= 0 || Math.abs(number - 1) < 0.00005 || Math.abs(number - Number(detail.originalUnitPrice ?? detail.unitPrice)) < 0.00005) {
                        window.alert('请输入大于 0、不能等于 1，且与原采购单价不同的有效单价。');
                        continue;
                    }
                    corrected = Number(number.toFixed(4));
                }

                if (corrected === null) break;
                result.details[detailIndex] = Object.assign({}, detail, {
                    unitPrice: corrected.toFixed(4),
                    manualUnitPrice: corrected.toFixed(4),
                    anomalyConfirmed: true,
                    anomalyConfirmedAt: new Date().toISOString(),
                });
                confirmedCount += 1;
            }
        }

        const recalculated = nextResults.map((result) => recalculateAggregatedResult(result, result.details));
        lastResults = recalculated;
        lastMeta = Object.assign({}, lastMeta || {}, buildAnomalyMeta(lastResults), { manualAnomalyReview: true });
        setResultCache(lastResults, lastMeta, capturedListUrl);
        renderAnomalyBanner();
        renderResults();
        renderStatus();
        ...
        if (hasPendingCostAnomalies(lastResults)) {
            showToast((cancelled ? '已停止修正，' : '') + '已确认 ' + confirmedCount + ' 条，剩余异常仍不能作为正式成本。', 'error');
        } else {
            dispatchCostResults(lastResults, lastMeta);
            showToast('全部成本异常已人工修正并确认，可以作为正式成本回传。');
        }
    }

    function updateActionState() {
        const disabled = lastResults.length === 0 || hasPendingCostAnomalies(lastResults);
    ...
    function createUi() {
        ...
        root.addEventListener('click', (event) => {
            if (event.target.closest('#erpa-review-anomalies')) reviewCostAnomalies();
        });
