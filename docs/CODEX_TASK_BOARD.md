...
## 2026-09-18 · 0.2.19 实现与本地回归完成

每日堆叠/月度簇状、期间扇形和 SKC 明细、门店销量 Top 5 与占比、相邻期间和返回状态保留已实现；利润页移除销售图。新增只读缓存模型，不迁移数据库或改变核算口径。110 文件 / 709 测试、前端构建、12 个桌面验证脚本通过，隔离浏览器十万行对账及浅深色/窄屏抽查通过。Windows 候选、CI、集成与公开发布仍待完成，不能视为已发布；本机未安装。详见 [实施与验收](integration/SALES_DETAIL_0.2.19_PLAN.md)。
