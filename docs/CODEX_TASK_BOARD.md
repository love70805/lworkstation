...
## 2026-09-22 · Beta.5 已实施，进入候选验收

用户已授权“执行本次升级”。A–F 工作包已整合：所选月份与整月状态贯通、有效报告投影、人工覆盖与 ERP 批次采用、商品统一保存校验、系统入口/备份落盘/更新取消，以及读取失败恢复和返回位置。补修重复成员初始化与 liveQuery 派生缓存落盘，1 万/10 万行两轮重开复用通过。实现分支 `codex/beta5-workflow-consistency`，基于 `d50343c`；未改真实数据，未新增迁移，本机不安装。最终测试、候选和公开发布状态见 [Beta.5 交付记录](RELEASE_0.3.0_BETA_5.md)；下方规划条目为历史阶段。
