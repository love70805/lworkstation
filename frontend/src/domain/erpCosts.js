...
function normalizeCostRow(row, index, defaultBatchId, resolutions) {
  ...
  return {
    ...
    previewUnitCost: previewCost?.toDecimalPlaces(6, Decimal.ROUND_HALF_UP).toNumber() ?? null,
    unitCost: decision?.unitCost ?? previewCost?.toDecimalPlaces(6, Decimal.ROUND_HALF_UP).toNumber() ?? null,
    formalUnitCost: decision?.formalUnitCost ?? (trustedPublishedLegacy ? previewCost.toDecimalPlaces(6, Decimal.ROUND_HALF_UP).toNumber() : null),
    currency,
...
export function calculateLegacyWarehouseCosts(records, { currentYearMonth = null, maxRecords = 3 } = {}) {
  ...
  const costs = [...buckets.entries()].map(([warehouseSku, bucket]) => {
    ...
    return {
      ...
      unitCost: totalPrice.div(totalQuantity).toDecimalPlaces(4, Decimal.ROUND_HALF_UP).toNumber(),
