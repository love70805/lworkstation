(function () {
    ...
    const EXTENSION_VERSION = '8.0.12';
    ...
    function setResultCache(results, meta, warehouseEvidence, capturedUrl) {
        try {
    ...
    function dispatchCostResults(results, meta, warehouseEvidence) {
        if (!Array.isArray(results) || results.length === 0) return;
        window.dispatchEvent(new CustomEvent('shopeers:erp-v8-cost-result', {
            detail: { results, meta, warehouseEvidence }
        }));
        return true;
    }
    ...
    async function runCalculation(filters, run) {
        ...
        const scopedWarehouseSkus = new Set(scopedState.results.map((result) => result.warehouseSku));
        const warehouseEvidence = {
            formatVersion: 1,
            warehouses: aggregateState.warehouseEvidence.warehouses.filter((item) => scopedWarehouseSkus.has(item.warehouseSku)),
            excludedOrders: detailState.excludedOrders,
            excludedDetails: aggregateState.warehouseEvidence.excludedDetails.filter((item) => (
                !item.warehouseSku || scopedWarehouseSkus.has(item.warehouseSku)
            )),
    ...
    async function calculate() {
        ...
        const state = await runCalculation(filters, run);
            ...
            setResultCache(lastResults, lastMeta, lastWarehouseEvidence, capturedListUrl);
            dispatchCostResults(lastResults, lastMeta, lastWarehouseEvidence);
            expandedRows.clear();
            ...
            updateActionState();
            hideLoading();
            showToast('已完成 ' + lastResults.length + ' 个仓库 SKU 预览并回传原始证据');
        } catch (error) {
    ...
    function createUi() {
        ...
        if (cachedResult) {
            ...
            window.setTimeout(() => dispatchCostResults(lastResults, lastMeta, lastWarehouseEvidence), 0);
        }
