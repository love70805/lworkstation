...
`0.2.6-beta.1` 起本地数据库 v11 升级到 v12 只执行 schema 前向迁移，不清空 ERP 请求、收件、正式成本、账本、利润结果或审计数据；已有业务数据和用户登录后的本地工作状态必须保留。后续数据净化或测试重置只能通过显式、受控的测试数据重置流程执行，不能藏在版本升级钩子中；日常 repository 流程仍拒绝修改锁定账本。
