...
## 2026-09-21 · Beta 临时历史三笔规则（实施中）

用户确认排除核算当月及以后，仅取核算月之前的采购；按时间由新到旧取最近三笔，不再优先 1688 单号。例如 b,a,b,b,a,a 采用 b,a,b。此临时口径取代下方 Beta.1 的含当月与 1688 优先规则，仅计划 `0.3.0-beta.2`，不升级稳定通道。数量加权、四位截断、异常处置与人工更正优先不变；旧采用成本若选样不同需复核，保留原金额，已定稿报告不改。

影响：ERP 域计算、正式成本持久化与缓存、成本核对表及利润导出、ERP 助手预览及 CSV。助手没有可信核算月份，只作未按账本筛月预览并保留完整证据。验收覆盖十二个月/跨年、b,a,b、同月和未来排除、旧采用证据、人工更正、定稿、导出及 Windows Electron。实施和验收中，尚未发布或安装。
