...
assert.deepEqual(updateConfig, { enabled: true, provider: "github", owner: "love70805", repo: "lworkstation", private: false, channel: "latest" });
for (const file of ["update-policy.cjs", "update-provider.cjs"]) assert.ok(pkg.build.files.includes(file), file);
assert.match(read("build-update-fixtures.mjs"), /"--publish",\s*"never"/);
assert.match(main, /if \(!updateRuntime\?\.canInstall\(\)\) return/);
assert.match(main, /if \(allowLoopback \|\| smokeUserDataPath \|\| smokeReportPath \|\| VISUAL_SMOKE\) return \{ enabled: false/);
