(function (global) {
  ...
  function normalizedExpectedSkus(values) {
    ...
    for (const item of (Array.isArray(values) ? values : [])) {
      ...
      result.set(canonical(platformSku), { platformSku, platformSkc });
    }
  ...
  function resolveUniqueRequest(records, {
    ...
    const candidates = (Array.isArray(records) ? records : []).filter((request) => {
      ...
      return normalizedQuerySkcs.length > 0 && normalizedQuerySkcs.every((skc) => normalizedSkcs(request?.platformSkcs).includes(skc));
    });
