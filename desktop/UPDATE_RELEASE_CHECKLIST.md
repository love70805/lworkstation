...
状态：beta.1 至 beta.5 已发布为 GitHub prerelease，beta.6 正在准备发布。生产稳定更新源继续关闭（`desktop/update-config.json` 的 `enabled: false`、`channel: latest`）。
...
- `0.2.6-beta.5 -> 0.2.6-beta.6`：本轮发布目标，仍需在 beta.5 安装环境中完成真实线上检查、下载和显式重启安装验收。
...
1. beta.1 至 beta.5 的 GitHub prerelease 保持可回读，资产文件名、大小、SHA-256 和 yml URL 必须一致。
2. 在隔离 Windows 测试机手工安装 beta.1 或当前 beta.5，确认更新浮窗处于 beta 测试通道且现有用户数据、登录态和 persist 分区不变。
3. 创建 `v0.2.6-beta.6` GitHub prerelease，上传 beta.6 的同名资产。
