...
用户截图确认 ERP 扩展混用台账之后的采购。正式成本层已按账本筛选，本轮补充可信 `ledgerPeriod` 传递及扩展预览、缓存、CSV 一致筛月，未关联月份不展示汇总价格。系统日期不作为成本截止月份。实现与验证进行中，尚未发布，本机不安装。见 [契约](integration/ERP_PREVIEW_LEDGER_MONTH_BETA_4.md)。
