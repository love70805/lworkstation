(function () {
    ...
    function getCache(key) {
        try {
            const raw = window.sessionStorage.getItem('erpAssistantV8_' + key);
            if (!raw) return null;
            const entry = JSON.parse(raw);
            if (!entry || Date.now() - Number(entry.timestamp) > CACHE_TTL) return null;
            return entry.value;
        } catch {
            return null;
        }
    ...
    function normalizeMappings(data) {
        ...
        data.forEach((item) => {
            ...
            if (!platformSku || seen.has(platformSku)) return;
            seen.add(platformSku);
            mappings.push({
                platformSku,
                storeName: String(item && (item.storeName || item.store || item.shopName) || '').trim()
            });
        });
        mappings.sort((a, b) => a.platformSku.localeCompare(b.platformSku, 'zh-CN', { numeric: true }));
        return mappings;
    }

    async function fetchAllOrders(filters, run) {
        setLoading('正在读取采购单', '读取第 1 页', 5);
        const firstResponse = await apiGetRetry(
            LIST_PATH,
        ...
        while (page <= MAX_LIST_PAGES) {
            ...
            setLoading(
                '正在读取采购单',
                '第 ' + page + ' 页 · 已读取 ' + uniqueOrders.size + (hasReportedCount ? ' / ' + reportedCount : '') + ' 条',
                hasReportedCount ? Math.min(24, 5 + Math.round((uniqueOrders.size / Math.max(reportedCount, 1)) * 19)) : 12
            );
        }
        if (!terminalPage) {
            throw new CostError(
    ...
    async function fetchAllDetails(orders, run) {
        const validOrders = orders.filter((order) => String(order.purchaseStatus) !== '11');
        ...
        const detailGroups = await mapConcurrent(
            ...
            async (order) => {
                ...
                if (Array.isArray(cached)) return cached.map((detail) => Object.assign({}, detail, {
                    _purchaseOrderId: id,
                ...
                try {
                    ...
                    const normalized = data.map((detail) => Object.assign({}, detail, {
                        _purchaseOrderId: id,
                        _orderNo1688: String(order.purchaseOrderNo1688 || '').trim(),
                        _purchaseOrderNo: String(order.purchaseOrderNo || '').trim()
                    }));
    ...
    async function fetchMappings(results, run) {
        ...
        const mappings = await mapConcurrent(
            ...
            async (result) => {
                ...
                const cached = getCache(cacheKey);
                if (Array.isArray(cached)) return { mappings: cached, failed: null };
                try {
                    const response = await apiGetRetry(
                        SKU_PATH,
    ...
    async function runCalculation(filters, run) {
        ...
        return {
            ...
            meta: {
                ...
                warehouseSkuCount: aggregateState.results.length,
                platformSkuCount: aggregateState.results.reduce((sum, item) => sum + item.mappings.length, 0),
    ...
    async function calculate() {
        ...
        try {
            ...
            lastMeta = state.meta;
            window.dispatchEvent(new CustomEvent('shopeers:erp-v8-cost-result', { detail: { results: lastResults, meta: lastMeta } }));
            expandedRows.clear();
    ...
    function buildExportRows(results) {
        ...
        results.forEach((result) => {
            const platformSkus = result.mappings.length > 0 ?
                result.mappings.map((mapping) => mapping.platformSku) : [result.warehouseSku];
    ...
    async function copyCosts() {
        ...
        const lines = ['平台SKU\t仓库SKU\t1688单号\t单件平均成本'];
        buildExportRows(lastResults).forEach((row) => {
            lines.push([row.platformSku, row.warehouseSku, row.orderNumber, row.unitCost].join('\t'));
        });
        try {
    ...
    function exportCsv() {
        if (lastResults.length === 0) return;
        const headers = [
            '仓库SKU', '平台SKU', '单号类型', '1688单号', '产品名称',
            '核算次数', '核算日期范围', '总采购量', '总采购价(￥)', '单件平均成本'
        ];
        const lines = [headers.map(csvCell).join(',')];
        buildExportRows(lastResults).forEach((row) => {
            lines.push([
                row.warehouseSku, row.platformSku, row.sourceType, row.orderNumber, row.name,
    ...
    function getFilteredResults() {
        ...
        return lastResults.filter((result) => {
            const mappings = result.mappings.map((mapping) => mapping.platformSku).join(' ');
            const haystack = [
                result.warehouseSku, result.name, result.orderNumber, result.sourceType, mappings
            ].join(' ').toLowerCase();
    ...
    function renderPlatformCell(result) {
        ...
        const first = escapeHtml(result.mappings[0].platformSku);
    ...
    function renderMappingList(result) {
        ...
        const visible = result.mappings.slice(0, 120).map((mapping) => escapeHtml(mapping.platformSku)).join(' · ');
        ...
        return '<div style="margin-bottom:10px;color:#5f6d80;line-height:1.7;word-break:break-all;">' +
            '<strong>平台映射：</strong>' + visible + (remainder > 0 ? ' · 其余 ' + remainder + ' 条请查看导出文件' : '') + '</div>';
    }
    ...
    function renderDetail(result) {
        const rows = result.details.map((detail) => (
            ...
            '<td class="erpa-cell-number erpa-cell-cost">' + escapeHtml(detail.unitPrice) + '</td></tr>'
        )).join('');
        return '<tr class="erpa-detail-row"><td colspan="9"><div class="erpa-detail-wrap">' +
            ...
            '<th>数量</th><th>总价(￥)</th><th>单价(￥)</th></tr></thead><tbody>' + rows +
    ...
    function renderResults() {
        ...
        body.innerHTML = results.map((result) => {
            ...
            const row = '<tr class="erpa-result-row' + (expanded ? ' erpa-expanded' : '') + '" data-sku="' + escapeHtml(key) + '">' +
                ...
                '<td class="erpa-cell-platform" title="' + escapeHtml(result.mappings.map((item) => item.platformSku).join(', ')) + '">' + renderPlatformCell(result) + '</td>' +
    ...
    function renderStatus() {
        ...
        if (!lastMeta) {
            updateIdleStatus();
            footerLeft.textContent = '数据链路：采购列表 → 采购明细 → 平台 SKU 映射';
        ...
        footerRight.textContent = '排除' + lastMeta.excludedMonth + ' · 1688单号优先 · 最近' + MAX_RECORDS + '单加权 · ' + (lastMeta.durationMs / 1000).toFixed(1) + '秒';
    }
    ...
    function createUi() {
        ...
        root.innerHTML =
            '<section class="erpa-panel" role="dialog" aria-modal="true" aria-label="SKU 采购成本核算">' +
                ...
                '<div class="erpa-toolbar">' +
                    '<input class="erpa-search" id="erpa-search" type="search" autocomplete="off" placeholder="搜索仓库 SKU、平台 SKU、产品名称或单号">' +
                ...
                '<div class="erpa-table-wrap" id="erpa-table-wrap"><table class="erpa-table">' +
                    ...
                    '<thead><tr><th>仓库 SKU</th><th>平台 SKU</th><th>1688 / 采购单号</th><th>产品名称</th>' +
        ...
        document.getElementById('erpa-recalculate').addEventListener('click', calculate);
        ...
        document.getElementById('erpa-error-retry').addEventListener('click', calculate);
    ...
    console.info(PREFIX, 'v8.0 已在采购管理页面启动');
}());
