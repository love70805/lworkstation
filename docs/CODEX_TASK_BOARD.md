...
### 2026-09-10 0.2.11 恢复经营概览（待发布）

用户要求恢复原经营概览；首页与独立利润页已恢复，保留全部 0.2.10 成本修复。86 文件582测试、构建及三尺寸桌面页面验收通过。主线准备分支 codex/restore-overview；正在集成和打包，本机不安装。见 [0.2.11 验收](RELEASE_0.2.11.md)。
...
- 当前公开 Latest：`0.2.10`；发布标签基线：`9e3a2d0`；本机仍为 `0.2.9`，用户选择暂不安装；本机安全 QA：`0.2.6-beta.8`（独立应用身份，未公开发布）
