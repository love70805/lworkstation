(function () {
    ...
    const EXTENSION_VERSION = '8.0.13';
    ...
    let capturedRequestContextPromise = null;
    let activeRun = null;
    let lastResults = [];
    let lastMeta = null;
    ...
    function inboxBaseUrl() {
        let stored = null;
        try { stored = localStorage.getItem('shopeers.erpInboxBaseUrl'); } catch { /* ignore storage errors */ }
        return [
            document.documentElement && document.documentElement.dataset ? document.documentElement.dataset.shopeersErpInboxBaseUrl : null,
            window.__SHOPEERS_ERP_INBOX_BASE_URL__,
            stored,
            DEFAULT_INBOX_BASE_URL
        ].map(validInboxBaseUrl).find(Boolean) || DEFAULT_INBOX_BASE_URL;
    }

    function configuredWorkspaceId() {
        let stored = '';
        try { stored = localStorage.getItem('shopeers.erpWorkspaceId') || ''; } catch { /* ignore storage errors */ }
        return String(
            document.documentElement?.dataset?.shopeersErpWorkspaceId
            || window.__SHOPEERS_ERP_WORKSPACE_ID__
            || stored
            || ''
        ).trim();
    }
    ...
    function loopbackTimeoutError() {
        return Object.assign(new Error('ERP 请求上下文读取超时，本地核算继续。'), {
            code: 'ERP_REQUEST_CONTEXT_TIMEOUT',
            status: 408
        });
    }
    ...
    async function fetchLoopbackJson(url, options = {}) {
        ...
        const operation = (async () => {
            ...
            try {
                ...
                throw Object.assign(new Error('ERP 请求上下文响应体不是有效 JSON。'), {
                    code: 'ERP_REQUEST_CONTEXT_INVALID_RESPONSE',
                    status: 502
                });
            }
    ...
    async function resolveRegisteredRequestContext(querySkcs, registeredBefore = new Date().toISOString(), workspaceId = configuredWorkspaceId()) {
        if (!nativeFetch || !Array.isArray(querySkcs) || querySkcs.length === 0) return null;
        const requestUrl = new URL(inboxBaseUrl() + '/erp/v1/requests');
        requestUrl.searchParams.set('registeredBefore', registeredBefore);
        ...
        if (!response.ok) {
            throw Object.assign(new Error('ERP 请求上下文读取失败：HTTP ' + response.status), {
                code: 'ERP_REQUEST_CONTEXT_HTTP_ERROR',
                status: response.status
            });
        }
        const resolved = requestContextPolicy.resolveUniqueRequest(payload.records, { workspaceId, querySkcs, registeredBefore });
        if (!resolved) throw requestContextPolicy.requestError('ERP_REQUEST_NOT_FOUND', '没有匹配查询快照和完整 SKC 集合的 ERP 请求。');
        return resolved;
    }
    ...
    function captureListRequest(rawUrl) {
        ...
        try {
            ...
            if (url.pathname === LIST_PATH) {
                ...
                capturedRequestContextPromise = resolveRegisteredRequestContext(extractQuerySkcs(filters), capturedQueryCapturedAt);
                void capturedRequestContextPromise.catch(() => {});
                updateIdleStatus();
    ...
    function dispatchCostResults(results, meta, warehouseEvidence, resultDeliveryId, deliveryState = {}) {
        ...
        window.dispatchEvent(new CustomEvent('shopeers:erp-v8-cost-result', {
            detail: {
                results,
                meta: deliveryMeta,
                warehouseEvidence,
                resultDeliveryId,
                requestId: deliveryMeta.requestId || '',
                ledgerId: deliveryMeta.ledgerId || '',
                workspaceId: deliveryMeta.workspaceId || '',
    ...
    window.addEventListener('shopeers:erp-v8-delivery-status', (event) => {
        ...
        if (detail.resultDeliveryId) {
            ...
            if (cached && cached.resultDeliveryId === detail.resultDeliveryId) {
                setResultCache(cached.results, cached.meta, cached.warehouseEvidence, cached.capturedUrl, cached.resultDeliveryId, {
                    ...
                    deliveryTerminal: Boolean(cached.deliveryTerminal || detail.terminal),
                });
            }
        }
        const messages = {
    ...
    async function fetchAllDetails(orders, run) {
        ...
        const isCancelledOrder = (order) => {
            return orderStatusValues(order).some((normalized) => {
    ...
    function aggregateDetails(details, now) {
        ...
        details.forEach((detail, detailIndex) => {
            ...
            const evidenceBase = {
                ...
                statusFields: {
                    purchaseStatus: detail.purchaseStatus ?? detail._purchaseStatus ?? null,
                    paymentStatus: detail.paymentStatus ?? detail.payStatus ?? detail._paymentStatus ?? null,
                    orderStatus: detail.orderStatus ?? detail._orderStatus ?? null,
                    order1688Status: detail.order1688Status ?? detail.orderStatus1688 ?? detail._order1688Status ?? null
                },
                eligible: false,
    ...
    async function runCalculation(filters, run) {
        ...
        const requestContext = run.requestContext || {};
        ...
        const warningMeta = buildWarningMeta(scopedState.results);
        ...
        const evidenceOnlySourceRecords = [
            ...
            ...aggregateState.warehouseEvidence.excludedDetails,
            ...mappingState.failedMappings,
        ];
        const evidenceOnlyResults = resultPolicy.buildEvidenceOnlyResults({
            unmappedResults: mappingPartition.evidenceOnly,
            sourceRecords: evidenceOnlySourceRecords,
            excludedWarehouseSkus: [...scopedWarehouseSkus],
        });
        const evidenceWarehouseSkus = new Set([
            ...scopedWarehouseSkus,
            ...evidenceOnlyResults.map((item) => resultPolicy.canonical(item.warehouseSku)),
        ]);
        const warehouseEvidence = {
            formatVersion: 1,
            warehouses: aggregateState.warehouseEvidence.warehouses.filter((item) => evidenceWarehouseSkus.has(resultPolicy.canonical(item.warehouseSku))),
            excludedOrders: detailState.excludedOrders,
            excludedDetails: aggregateState.warehouseEvidence.excludedDetails.filter((item) => (
                !item.warehouseSku || evidenceWarehouseSkus.has(resultPolicy.canonical(item.warehouseSku))
            )),
            detailFailures: detailState.failedOrders,
            mappingFailures: mappingState.failedMappings
        };
        const previewResults = [...scopedState.results, ...evidenceOnlyResults];
        return {
            results: previewResults,
            warehouseEvidence,
            meta: {
                requestId: requestContext.requestId || '',
                ledgerId: requestContext.ledgerId || '',
                workspaceId: requestContext.workspaceId || configuredWorkspaceId(),
                requestRegisteredAt: requestContext.requestRegisteredAt || '',
                queryCapturedAt: run.queryCapturedAt,
                registeredBefore: run.queryCapturedAt,
                requestContextError: run.requestContextError,
                filters,
                querySkcs,
    ...
    async function calculate() {
        ...
        try {
            const calculationState = await requestContextPolicy.runWithOptionalRequestContext(
                capturedRequestContextPromise || resolveRegisteredRequestContext(extractQuerySkcs(filters), run.queryCapturedAt),
                (contextState) => {
                    run.requestContext = contextState.context;
                    run.requestContextError = contextState.error;
                    if (run.requestContextError) console.warn(PREFIX, '自动回传上下文异常，本地核算继续', run.requestContextError);
                    return runCalculation(filters, run);
                },
                LOOPBACK_REQUEST_TIMEOUT_MS + 250
            );
            const state = calculationState.result;
            if (run.cancelledByUser) return;
            lastResults = state.results;
            lastMeta = state.meta;
            lastWarehouseEvidence = state.warehouseEvidence;
            const resultDeliveryId = makeResultDeliveryId();
            const deliveryState = {
            ...
            dispatchCostResults(lastResults, lastMeta, lastWarehouseEvidence, resultDeliveryId, deliveryState);
            expandedRows.clear();
            ...
            updateActionState();
            hideLoading();
            showToast(run.requestContextError
                ? '核算已完成；自动回传上下文异常，本地预览、复制和 CSV 仍可使用'
                : '已完成 ' + lastResults.length + ' 个仓库 SKU 预览并回传原始证据');
        } catch (error) {
    ...
    function renderResults() {
        ...
        body.innerHTML = results.map((result) => {
            ...
            const costState = warnings.count > 0
    ...
    function createUi() {
        ...
        trigger.type = 'button';
        trigger.innerHTML = '<span class="erpa-trigger-icon" aria-hidden="true">▦</span><span>预览 SKU 成本</span>';
        ...
        if (cachedResult) {
            ...
            if (requestContextPolicy.shouldReplayDelivery(cachedResult)) {
                window.setTimeout(() => dispatchCostResults(lastResults, lastMeta, lastWarehouseEvidence, resultDeliveryId, cachedResult), 0);
            }
        }
    ...
    function reportExtensionStatus() {
        ...
        nativeFetch(inboxBaseUrl() + '/erp/v1/extension-status', {
            method: 'POST',
