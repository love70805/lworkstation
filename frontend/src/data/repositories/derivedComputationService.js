...
export async function readCachedReportProducts({ snapshot, warehouseRate = snapshot?.ledger?.warehouseRate, onStatus }) {
  ...
  return cachedDerived({ scope: [snapshot.ledger.workspaceId, snapshot.ledger.id, 'all', String(warehouseRate)], formula: `${REPORT_FORMULA_VERSION}:products@2-preview`, revision: snapshot.dataVersion, compute, onStatus });
}
