...
- 阶段交付：人工/ERP/首页/安装向导/弹窗及两轮审查修订已承接准备分支；主线最终 release:check 86 文件581测试/构建及全部门禁通过，desktop verify 11组通过；实际浏览器微小/零/跨店/撤销/定稿/重开和三尺寸首页验收通过。用户已明确纯本机交付，本轮不追加 SQL 0012；云端人工成本兼容不在交付范围。未生成正式候选、未合入受保护集成分支、未发布新版本。详见 [验收记录](RELEASE_0.2.10.md)。
...
- 当前公开 Latest：`0.2.9`；发布标签基线：`62b9075`；本分支准备版号 `0.2.10` 尚未发布；本机安全 QA：`0.2.6-beta.8`（独立应用身份，未公开发布）
