...
export function hasLegacyMonthExclusions(cost, period) {
  ...
  const records = [...adoptedCostEvidence(cost).records, ...(evidence?.excludedRecords ?? [])];
  ...
  // A record kept only in excludedRecords is not part of the complete evidence
  // used for selection. Require recollection if an old cutoff omitted it.
  if ((evidence?.excludedRecords ?? []).some(record => {
    if (!record.exclusionReasons?.some(reason => ["on_or_after_ledger_period", "after_ledger_period"].includes(reason))) return false;
    const month = purchaseRecordPeriod(record);
    return month === null || month <= period;
  })) return true;
