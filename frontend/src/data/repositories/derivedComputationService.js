...
export async function readCachedReportProducts({ snapshot, warehouseRate = snapshot?.ledger?.warehouseRate, onStatus }) {
  ...
  return cachedDerived({ scope: [snapshot.ledger.workspaceId, snapshot.ledger.id, snapshot.ledger.period, 'all', String(warehouseRate)], formula: `${REPORT_FORMULA_VERSION}:products@5-beta-current-month-latest-three`, revision: snapshot.dataVersion, compute, onStatus });
}
