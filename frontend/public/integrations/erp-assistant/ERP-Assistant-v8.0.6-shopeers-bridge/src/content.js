(function () {
    ...
    const RESULT_CACHE_KEY = 'latest_cost_result';
    const PREFIX = '[ERP Assistant]';
    const EXTENSION_VERSION = '8.0.6';
    ...
    async function fetchAllOrders(filters, run) {
        ...
        while (page <= MAX_LIST_PAGES) {
            ...
            page += 1;
            const totalPages = hasReportedCount ? Math.max(page, Math.ceil(reportedCount / pageSize)) : null;
            setLoading(
                '正在读取采购单',
                totalPages ? '正在读取订单列表… ' + page + '/' + totalPages + ' 页' : '正在读取订单列表… 第 ' + page + ' 页',
