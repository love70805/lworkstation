...
export default function ManualCostDialog({ ledger, row, onClose, onNext, children, readOnly = false, className = "", title = "人工更正成本" }) {
  ...
  return <Modal open title={title} className={className} description={`${ledger.period} · ${row.store} · ${row.platformSku}。${readOnly ? "已定稿或锁定，当前只读。" : "人工更正仅对当前店铺月份生效；撤销后恢复最新可用 ERP 或缺失状态。"}`} onClose={() => !saving && onClose()}
