...
用户更正并确认所有月份均使用该月及以前采购，禁止后续月份。正在修改采集证据保留、成本核对、采用时独立复算和旧成本日期检查；最近三条／1688 优先／数量加权规则不变，人工更正优先，已定稿报告不静默改写。契约与验收见 [月末截止](integration/ERP_LEDGER_MONTH_CUTOFF.md)。尚未发布新包。
