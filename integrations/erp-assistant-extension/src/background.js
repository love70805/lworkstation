(() => {
  ...
  async function submitCostResult(input, sender) {
    if (!senderAllowed(sender)) throw loopbackError('ERP_UNTRUSTED_SENDER', '只允许 ERP 采购管理页的顶层隔离脚本投递。', 403);
    let config;
    try {
  ...
  async function reportInstalled({ ready = false, sender = null } = {}) {
    const pageUrl = senderAllowed(sender) ? String(sender.tab?.url || sender.url || '') : '';
