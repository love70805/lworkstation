(function () {
    ...
    const EXTENSION_VERSION = '8.0.21';
    ...
    async function readPreviewContext(querySkcs, queryCapturedAt) {
        ...
        return response?.ok && resultPolicy.validLedgerPeriod(response.ledgerPeriod)
            ? { ledgerPeriod: response.ledgerPeriod }
    ...
    function dispatchCostResults(results, meta, warehouseEvidence, resultDeliveryId, deliveryState = {}) {
        ...
        void bridge.submit({
            ...
            queryCapturedAt,
            registeredBefore: queryCapturedAt
        }).then(handleDeliveryStatus, (error) => handleDeliveryStatus({
            status: 'cached',
            resultDeliveryId,
    ...
    async function runCalculation(filters, run) {
        ...
        const previewContext = await readPreviewContext(querySkcs, run.queryCapturedAt);
        const orderState = await fetchAllOrders(filters, run);
        const orders = orderState.orders;
        const detailState = await fetchAllDetails(orders, run);
        setLoading('正在整理采购证据', previewContext.ledgerPeriod ? `台账月份：${previewContext.ledgerPeriod}` : '未取得台账月份，继续采集完整证据', 65);
        ...
        const previewResults = resultPolicy.previewForLedger([...scopedState.results, ...evidenceOnlyResults], warehouseEvidence, previewContext.ledgerPeriod);
    ...
    async function calculate() {
        ...
        try {
            ...
            updateActionState();
            hideLoading();
            showToast('采集完成，共 ' + lastResults.length + ' 个仓库 SKU；送达状态以回传提示为准');
        } catch (error) {
    ...
    function previewRowStatus(result) {
        return result.previewStatus === 'period_unknown' ? '台账月份待关联' : result.previewStatus === 'no_purchase' ? '台账当月及以前无可用采购' : result.previewStatus === 'evidence_missing' ? '采购证据待补充' : '按台账预览';
    }
    ...
    function createUi() {
        ...
        if (existingTrigger && existingRoot) return true;
        ...
        if (cachedResult) {
            ...
            void restorePreviewContext(cachedResult);
        }
    ...
    function scheduleUiRepair() {
        if (document.getElementById('erpa-cost-trigger') && document.getElementById('erpa-cost-root')) return;
